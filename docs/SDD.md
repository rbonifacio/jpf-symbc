...
Reference document for the Spec-Driven Development approach adopted by SDD Toolkit. This document serves as a self-contained guide — a developer reading only this document should understand what SDD is, why it exists, how it works, where the tools are, what the criticisms are, and how SDD Toolkit applies it.

**Approach**: SDD Toolkit follows **spec-anchored SDD** — specifications persist and document the system, but code remains the maintained artifact. See Martin Fowler's spectrum model in Section 5.
...
11. [SDD in the SDD Toolkit](#11-sdd-in-the-sdd-toolkit)
...
- **SDD Toolkit's adoption**: How this project applies SDD (Section 11)

For the project-specific workflow details (tracks, phases, skill sequences), see `docs/SDD-WORKFLOW.md`. For the product requirements, see `docs/PRD.md`.
...
| **Integration contracts** | How components interact | "The analysis skill communicates with the MCP server via the ToolRegistry interface" |
...
**Concrete example**: In the SDD Toolkit, a domain spec (e.g., `openspec/specs/workflow/spec.md`) defines scenarios and invariants that describe expected behavior. When a new Claude Code session begins, the agent can read this spec and immediately understand the expected behavior — without anyone having to re-explain how the workflow should work.
...
**Example**: SDD Toolkit's approach — PRD, domain specs, and change artifacts persist in `openspec/` and `docs/`, but the implementation code is the executed artifact and source of truth.
...
### 5.7 SDD Toolkit's Position

SDD Toolkit uses **spec-anchored SDD**. This was chosen because:

1. **Brownfield codebase**: The SDD Toolkit has 51 skills and agents with 53 functional requirements — it is not a greenfield project
...
**Solutions**:
- **Divide by domain**: Separate backend and frontend specs, or separate specs per module (as the SDD Toolkit does with domain specs per capability)
...
**They are complementary, not competing**: In SDD Toolkit, the `/sdd-tdd` skill operates *within* the SDD workflow. The specification defines *what* to build; TDD defines *how to verify* it. The `tasks.md` artifact in an OpenSpec change typically includes test tasks. You can (and should) use both.
...
**SDD borrows heavily from BDD**: ThoughtWorks notes that "specification principles from behavior-driven development remain valid — domain language, structured scenarios, and living documentation practices translate effectively to AI-assisted workflows." Many SDD specifications use Given/When/Then format directly. SDD Toolkit's specs use WHEN/THEN/AND format with RFC 2119 keywords — a clear BDD inheritance.
...
In SDD Toolkit, this is how the `/sdd-tdd` skill works within the SDD workflow. The OpenSpec change defines *what* to build; the TDD cycle ensures each piece works correctly.
...
This is the framework adopted by SDD Toolkit. See Section 11 for detailed usage.
...
**Counter-arguments**:
- **Proportional ceremony** addresses the "1,300 lines for a date feature" problem. SDD implementations with tiered workflows (like SDD Toolkit's Quick Path) do not require spec artifacts for mechanical changes — a lightweight `plan.md` suffices.
...
**Mitigation**: Concise specification templates, structured sections (tables over prose), and proportional ceremony (small changes need small specs). SDD Toolkit's specs use WHEN/THEN/AND format and tables rather than verbose prose.
...
**Mitigation**: Tiered workflows. SDD Toolkit's Quick Path (plan.md only, no spec artifacts) is used for mechanical changes to existing code where the code itself is sufficient context. SDD ceremony is reserved for changes where design decisions benefit from the additional structure.
...
## 11. SDD in the SDD Toolkit

This section documents how the SDD Toolkit applies SDD in practice. It is specific to this project but illustrates how the generic principles from earlier sections translate into a real workflow built on Claude Code skills, agents, and an MCP-based knowledge infrastructure.

### 11.1 Why We Adopted SDD

The SDD Toolkit is a research artifact developed as part of a PhD program at UnB (Universidade de Brasilia), following Design Science Research (DSR) methodology. The goal is to produce a generic, language-agnostic toolkit that brings Spec-Driven Development to any codebase through Claude Code skills and agents.

The predecessor project, rv-android, had 46 Python-specific skills. Those skills proved that SDD works in practice but were tightly coupled to a single language and project. The SDD Toolkit migrates that experience into a portable system with 53 functional requirements (documented in `docs/PRD.md`) covering infrastructure, parsers, analysis, orchestration, documentation, and setup.

The adoption of SDD for developing the toolkit itself was motivated by five needs:

| Need | Problem Without SDD | Solution With SDD |
|------|---------------------|-------------------|
...
| **Academic rigor** | Informal documentation insufficient for PhD research | Formal specs with invariants and scenarios |

### 11.2 Our Position: Spec-Anchored

The SDD Toolkit uses spec-anchored SDD per Fowler's spectrum:

- **Specifications persist**: PRD (`docs/PRD.md`), domain specs (`openspec/specs/*/spec.md`), and change artifacts persist in version control
- **Code is authoritative**: The skill definitions, MCP server code, and infrastructure scripts are the executed artifacts and source of truth
- **Specs document current behavior**: OpenSpec's `config.yaml` includes the rule: "Specs document the CURRENT behavior of the system — not aspirational or historical state"
...
The SDD Toolkit uses OpenSpec by Fission-AI as the SDD process layer. OpenSpec was chosen for:

| Criterion | Why OpenSpec |
|-----------|-------------|
| **Brownfield-first** | Designed for existing codebases — the toolkit has 53 FRs across multiple tiers |
| **Tool-agnostic** | Works with Claude Code (our primary tool) and 20+ other platforms via adapters |
| **Fluid workflow** | No rigid phase gates — matches the iterative research workflow |
...
schema: sdd-full
context: |
  Project: SDD Toolkit (agentes-claude)
  Type: Generic, language-agnostic Spec-Driven Development toolkit
  Stack: Claude Code skills/agents, Python MCP server (sdd-mcp), Neo4j, tree-sitter
  Architecture: 4-tier (Agents → Orchestrators → Analysis → Support) + Infrastructure

  DEVELOPMENT PRINCIPLES (see docs/SDD-WORKFLOW.md):
  P1 Simplicity — minimal complexity, no premature abstractions
  P2 Human-readable — deep narrative docs, self-contained for implementers
  P3 No backward compatibility — delete dead code, no adapters/shims
  P4 Current-state comments — no migration history, no promotional language

  Skill prefix: sdd-*
  Operating modes: Full (Neo4j), Lite (MCP Memory), Minimal (Claude reads code)
rules:
  specs:
    - "Specs document the CURRENT behavior of the system — not aspirational or historical state"
    - "Use RFC 2119 keywords (SHALL, SHOULD, MAY) for requirements"
    - "Write scenarios in WHEN/THEN/AND format"
    - "Invariants are verifiable assertions, prefixed with INV-<DOMAIN>-<NN>"
  tasks:
    - Include a final Verification group
    - Annotate skill invocations as checkbox items where they add value
```
...
├── config.yaml                         # Project context and rules
├── schemas/
│   ├── sdd-full/                       # Full SDD schema
│   │   ├── schema.yaml                 # Phase definitions and artifact requirements
│   │   └── templates/                  # proposal.md, spec.md, design.md, tasks.md
│   └── sdd-quick-path/                # Quick Path schema
│       ├── schema.yaml                 # Simplified phase definitions
│       └── templates/                  # plan.md, tasks.md
├── specs/                              # Domain specifications (current behavior)
│   ├── development-workflow/spec.md    # Workflow tracks, phases, skill sequences
│   ├── sdd-methodology-docs/spec.md   # SDD methodology and documentation standards
│   └── tracing-hooks/spec.md          # Hook-based tracing infrastructure
└── changes/                            # Change artifacts (proposals, delta specs, designs, tasks)
    └── archive/                        # Completed changes
```

Each domain spec covers a group of related concerns and maps to specific functional requirements from the PRD. The `sdd-full` schema governs changes that require design decisions (proposal, specs, design, tasks), while the `sdd-quick-path` schema governs mechanical changes (plan and tasks only).

### 11.4 The Two-Layer Architecture

The SDD Toolkit integrates SDD through two distinct layers: the **process layer** (OpenSpec) and the **execution layer** (sdd-* skills). This separation is intentional — the process layer manages *what* to build and *why*; the execution layer handles *how*.
...
flowchart TD
    ...
    subgraph Execution["Execution Layer (sdd-* skills)"]
        subgraph Agents["2 Agents"]
            direction LR
            WF["sdd-workflow"]
            RV["sdd-reviewer"]
        end
        subgraph Orchestrators["13 Orchestrators"]
            direction LR
            AR["sdd-architect"]
            DC["sdd-documenter"]
            TS["sdd-tester"]
            ...
            CL["sdd-cleanup"]
            PL["sdd-planning"]
            CR["sdd-code-reviewer"]
            SE["sdd-security"]
            VE["sdd-verify"]
        end
        subgraph Analysis["6 Analysis"]
            direction LR
            AF["sdd-analyze-file"]
            AM["sdd-analyze-module"]
            AC["sdd-analyze-complexity"]
            AD["sdd-analyze-dead-code"]
            ADP["sdd-analyze-dependencies"]
            IA["sdd-impact-analyzer"]
        end
        subgraph Documentation["7 Documentation"]
            direction LR
            DA["sdd-doc-architecture"]
            DADR["sdd-doc-adr"]
            DN["sdd-doc-nfr"]
            DCO["sdd-doc-code"]
            DR["sdd-doc-readme"]
            DG["sdd-doc-generate-claude-md"]
            DS["sdd-docs-sync"]
        end
        subgraph Support["10 Support"]
            direction LR
            QL["sdd-qa-lint\nsdd-qa-lint-fix"]
            TR["sdd-test-run\nsdd-test-add"]
            DB["sdd-debug-regression"]
            RL["sdd-release"]
            RT["sdd-retrospective"]
            RK["sdd-risk"]
            EX["sdd-extract"]
            SU["sdd-setup"]
        end
        subgraph Foundation["3 Foundation"]
            direction LR
            ME["sdd-methodology"]
            DE["sdd-detection"]
            CG["sdd-config-reader"]
        end
    end

    subgraph Gate["Quality Gate"]
        GCR["sdd-code-reviewer\n(auto-chained by orchestrators)"]
    end

    Process -->|"delegates implementation to"| Orchestrators
    Process -->|"uses for analysis"| Analysis
    Agents -->|"orchestrate"| Orchestrators
    Orchestrators -->|"invoke"| Analysis
    Orchestrators -->|"invoke"| Support
    Orchestrators -->|"chain to"| GCR
    Analysis -->|"read"| Foundation
```

**Design principle**: Unidirectional flow — the process layer invokes the execution layer, never the reverse. This keeps sdd-* skills reusable independently of OpenSpec. A developer can use `/sdd-tdd` without SDD; but the SDD workflow can also call `/sdd-tdd` for test-driven tasks.
...
Bockeler (Martin Fowler, 2026) documented the anti-pattern of applying SDD uniformly: Kiro turned a small bug fix into "4 user stories with 16 acceptance criteria" — a "sledgehammer to crack a nut." File count alone does not determine the track: a 45-file dead module removal is Quick Path if the plan is clear and no design decisions are needed.
...
| Track | When to Use | Schema | Change Directory | Example |
|-------|-------------|--------|------------------|---------|
| **Full SDD** | Design decisions + multi-module or architectural | `sdd-full` | `openspec/changes/` with full artifacts (proposal, delta specs, design, tasks) | Adding a language parser plugin that touches parsers + ontology + MCP tools |
| **Fast-Forward SDD** | Design decisions + single module, clear requirements | `sdd-full` | `openspec/changes/` with full artifacts (auto-generated via `/opsx:ff`) | Adding a new aggregation query to sdd-mcp |
| **Quick Path** | No design decisions — mechanical, clear plan | `sdd-quick-path` | `openspec/changes/` with `plan.md` and `tasks.md` only | Fixing an analysis skill bug, removing dead code |
...
| Does it cross tier boundaries with architectural implications? | Full SDD |
| Is it a single-skill or single-module change with spec implications? | FF SDD |
...
| Are requirements clear and the task is mechanical? | **Quick Path** |

When in doubt, start with Quick Path. You can escalate to a higher track if the change turns out to need design decisions.

### 11.6 Example: Full SDD — Adding a Language Parser Plugin

This is a multi-component change (parsers, ontology mapper, MCP tools, language plugin config) introducing a new language to the toolkit. It goes through all 6 phases.

```bash
# ─── Phase 1: Explore ───────────────────────────────────────────
# Goal: Understand the problem and assess impact
/sdd-analyze-module mcp-server/src/sdd_mcp/parsers   # Understand parser architecture
/sdd-analyze-dependencies mcp-server                   # Map parser → ontology → tools chain
/opsx:explore                                          # Document findings, think through approach

# ─── Phase 2: Propose ───────────────────────────────────────────
# Goal: Create formal change proposal with delta spec outlines
/opsx:new add-go-parser                   # Create change directory + initial proposal
/opsx:continue                            # Write delta specs for parser interface
/opsx:continue                            # Write delta specs for ontology mapping

# ─── Phase 3: Design ────────────────────────────────────────────
# Goal: Write design document and task list
/opsx:continue                            # Generate design document (interfaces, patterns)
...
/opsx:apply                               # Start implementing tasks
  ...
  # /sdd-tdd for the new Go parser class (test-first)
  # /sdd-feature for ontology mapping and plugin config

# ─── Phase 5: Verify ────────────────────────────────────────────
# Goal: Validate implementation against specs
/sdd-verify mcp-server                   # Run tests + lint + type checks
...
### 11.7 Example: Quick Path — Fixing an Analysis Skill Bug

A bug in `sdd-analyze-complexity` produces incorrect cyclomatic complexity for nested functions. No design decisions needed.

```bash
# ─── Phase 1: Analyze ───────────────────────────────────────────
# Goal: Understand the bug
/sdd-debug-regression test_nested_complexity   # Investigate via git history and tests

# ─── Phase 2: Fix ───────────────────────────────────────────────
# Goal: Fix the bug with test coverage
/sdd-tdd                                       # Write failing test, then fix code

# ─── Phase 3: Verify ────────────────────────────────────────────
# Goal: Confirm the fix
/sdd-verify sdd-analyze-complexity             # Run all checks (tests + lint)
```
...
The SDD Toolkit supports three operating modes that determine how skills access project knowledge. This allows the toolkit to work in environments ranging from a full Docker + Neo4j setup to a plain Claude Code session with no external dependencies.

```mermaid
flowchart TD
    START([Install SDD Toolkit]) --> CHECK_DOCKER{Docker +\nPython available?}

    CHECK_DOCKER -->|Yes| FULL["<b>Full Mode</b>\nNeo4j graph via sdd-mcp\nRichest analysis"]

    CHECK_DOCKER -->|No| CHECK_MCP{MCP Memory\navailable?}

    CHECK_MCP -->|Yes| LITE["<b>Lite Mode</b>\nMCP Memory entities/relations\nCached analysis"]

    CHECK_MCP -->|No| MINIMAL["<b>Minimal Mode</b>\nClaude reads code directly\nNo persistence"]

    FULL --> PERSIST_F["Persistence: Neo4j graph + .sdd/cache/"]
    LITE --> PERSIST_L["Persistence: MCP Memory entities"]
    MINIMAL --> PERSIST_M["Persistence: None (session only)"]

    style FULL fill:#e3f2fd,stroke:#1565c0
    style LITE fill:#fff3e0,stroke:#ff9800
    style MINIMAL fill:#e8f5e9,stroke:#2e7d32
```

| Mode | Requirements | Knowledge Source | Persistence |
|------|-------------|-----------------|-------------|
| **Full** | Docker, Python | Neo4j graph via sdd-mcp (tree-sitter parsing, ontology mapping, structural queries) | Graph + `.sdd/cache/` |
| **Lite** | Claude Code only | MCP Memory (`mcp__memory__*` tools) for cached analysis results | Entities/relations in memory |
| **Minimal** | Claude Code only | Claude reads code directly via Read/Glob/Grep | None (session only) |

Each analysis skill follows this pattern:

```
IF mode == full:
  → query sdd-mcp tools (get_component, get_dependencies, etc.)
ELIF mode == lite:
  → check MCP Memory for cached analysis
  → if cache miss: Claude reads code → cache in MCP Memory
ELSE (minimal):
  → Claude reads code directly (no cache)
```

Mode detection reads `.sdd/sdd-config.yaml` at the target project root. If the file is missing, the skill assumes Minimal mode.

### 11.9 SDD Artifact Inventory

A complete inventory of all SDD-related artifacts in the SDD Toolkit project:

| Artifact | Path | Content |
|----------|------|---------|
| **PRD** | `docs/PRD.md` | 53 functional requirements, 9 non-functional requirements |
| **Domain specs** | `openspec/specs/*/spec.md` | Current system behavior, invariants, scenarios |
| **OpenSpec config** | `openspec/config.yaml` | Project context, rules, brownfield constraints |
| **Custom schemas** | `openspec/schemas/sdd-full/`, `openspec/schemas/sdd-quick-path/` | Phase definitions, templates for each workflow track |
| **Workflow guide** | `docs/SDD-WORKFLOW.md` | Track selection, phase details, skill sequences |
| **SDD reference** | `docs/SDD.md` | This document |
| **Implementation plan** | `docs/pre-plano.md` | Architecture, skill inventory, MCP server design, implementation phases |
| **Skill/agent rationale** | `docs/skills-agents-architecture.md` | Nesting constraints, tier mapping, portability analysis |
| **Templates** | `openspec/schemas/*/templates/` | Templates for proposals, specs, designs, tasks, plans |

### 11.10 Validating Our Workflow Against SDD Best Practices

How does the SDD Toolkit's workflow align with the principles and best practices described in this document?

| Best Practice | SDD Toolkit Implementation | Status |
|--------------|--------------------------|--------|
| **Proportional ceremony** (Section 4.6) | Three tracks: Full SDD, Fast-Forward, Quick Path; two schemas (`sdd-full`, `sdd-quick-path`) | Implemented |
...
| **Human-in-the-loop** (Section 4.3) | Review checkpoints at each phase via orchestrator user checkpoints | Implemented |
| **TDD integration** (Section 7.5) | `/sdd-tdd` skill within SDD workflow | Implemented |
...
| **Mode degradation** (NFR04) | Full → Lite → Minimal with graceful fallback | Implemented |
| **Cross-platform portability** (NFR02) | 39 skills follow agentskills.io standard (26+ platforms) | Designed |
...
| Document | Purpose |
|----------|---------|
| `docs/PRD.md` | Product Requirements Document (53 FRs, 9 NFRs) |
| `docs/SDD-WORKFLOW.md` | Authoritative workflow reference (tracks, phases, skill sequences) |
| `docs/pre-plano.md` | Implementation plan (skill inventory, architecture, MCP server design) |
| `docs/skills-agents-architecture.md` | Skill/agent design rationale (nesting constraints, portability) |
